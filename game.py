...
def scale(box):
    return pygame.Rect(box.x * 10 + 1800, box.y * 10 + 1500, box.w * 10, box.h * 10)

def draw_street(street, file_name):
    pygame.init()
    screen = pygame.display.set_mode((1, 1)) #, pygame.RESIZABLE)
    img = pygame.Surface((3600, 1700))
    img.fill((0, 148, 255))
    if street.bg_street.bg_street:
        for building in street.bg_street.bg_street.buildings:
            if building.silhouette:
                pygame.draw.rect(img, (30, 30, 30), scale(building.visual_box))
    for building in street.bg_street.buildings:
        if building.silhouette:
            pygame.draw.rect(img, (50, 50, 50), scale(building.visual_box))

    for building in street.buildings:
        img.blit(BuildingTextures.get(building.building_type), scale(building.visual_box))
